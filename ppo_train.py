...
def train_ppo(episodes: int, gamma: float, lr: float, checkpoint: str = None, save_period: int = 10, n_envs: int = 1):
    """Train PPO-Clip (actor-only) on Bowling with batch-only vector rollouts."""
    from ppo import episode_termination, policy
...
def eval_policy(checkpoint: str, episodes: int = 5):
    ...
    for ep in range(episodes):
        state, _ = ppo.env.reset()
        state = state.to(DEVICE)
        rewards = []

        with torch.no_grad():
            while True:
                probs = ppo.policy.get_action_probabilities(state.unsqueeze(0))
                action = torch.multinomial(probs, 1).item()
                next_state, reward, done, _, _ = ppo.env.step(action)
                next_state = next_state.to(DEVICE)
                rewards.append(reward)

                # Рисуем с нашим персонажем
                frame = replace_bowler(ppo.env.env.render(), dude)
                ax.clear()
                ax.imshow(frame)
                ax.axis("off")
